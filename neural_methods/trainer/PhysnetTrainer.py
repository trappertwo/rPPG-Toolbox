...
from neural_methods.model.SwinPhys import SwinPhys
...
class PhysnetTrainer(BaseTrainer):
    ...
    def __init__(self, config, data_loader):
        ...
        if config.MODEL.PHYSNET.USE_SWINPHYS:
            swinir_model_path = config.MODEL.PHYSNET.SWINIR_MODEL_PATH
            # SwinIR pretrained model has image size 126
            self.model = SwinPhys(swinir_model_path, restore=config.MODEL.PHYSNET.DO_RESTORE, physnet_model_path=config.INFERENCE.MODEL_PATH,
                                  diff_normalize=config.MODEL.PHYSNET.SWINPHYS_DIFF_NORMALIZE, window_size=7, img_size=126, num_frames=config.MODEL.PHYSNET.FRAME_NUM,
                                  freeze_swinir=config.MODEL.PHYSNET.FREEZE_SWINIR, freeze_physnet=config.MODEL.PHYSNET.FREEZE_PHYSNET,
                                  model_type=config.MODEL.PHYSNET.SWIN_MODEL_TYPE) # [3, T, 128,128]
        else:
    ...
    def test(self, data_loader):
        ...
        if self.config.TOOLBOX_MODE == "only_test":
            ...
            if not self.config.MODEL.PHYSNET.USE_SWINPHYS:
